...
<script>
  ...
  export default{
    ...
    methods:{
      init(){
        this.mealFlag=  this.$route.params.meal == '1'?true:false
      ...
      getCourseDetail(){
        let self = this;
        self.$http.get('/privateClass/classInfoByClassId?classId='+this.$route.params.id).then(function (res) {
          if(res.result==1){
      ...
      /*预约课程*/
      submitHandler(){
        let url='/privateClass/apply?classId='+this.$route.params.id+'&timeId='+this.$route.params.time+'&memberId='+localStorage.getItem('fitId')
        let self = this;
